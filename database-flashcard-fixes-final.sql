...
BEGIN
    -- 安全地添加其他列到user_answers表（除了session_id）
    ...
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='user_answers' AND column_name='session_id' AND data_type != 'text') THEN
        ...
        LOOP
            EXECUTE 'ALTER TABLE public.user_answers DROP CONSTRAINT ' || constraint_name;
        ...
        RAISE NOTICE 'Changed session_id column type to TEXT';
    END IF;
    
    -- 添加新的session_id_text列作为替代方案（如果需要保留原有数据结构）
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='user_answers' AND column_name='session_id_text') THEN
        ...
        RAISE NOTICE 'Added session_id_text column as alternative';
    END IF;
...
DO $$
BEGIN
  -- 检查demo用户是否存在于auth.users表中，如果不存在则跳过
  IF EXISTS (SELECT 1 FROM auth.users WHERE id = 'demo-user-123'::UUID) THEN
  ...
  ELSE
    RAISE NOTICE 'Demo user does not exist in auth.users, skipping flashcard progress insertion';
  END IF;
END $$;
...
BEGIN
    RAISE NOTICE '===========================================';
    RAISE NOTICE 'Flashcard database upgrade completed successfully!';
    RAISE NOTICE '===========================================';
    RAISE NOTICE 'Created tables:';
    RAISE NOTICE '  ✓ user_flashcard_progress';
    RAISE NOTICE 'Enhanced tables:';
    RAISE NOTICE '  ✓ flashcards (added vocabulary fields)';
    RAISE NOTICE '  ✓ user_answers (added review fields)';
