-- Final fix for flashcards table with proper UUID handling
-- This script handles the UUID user_id requirement properly
...
-- Create a demo user UUID for testing (if it doesn't exist)
DO $$
DECLARE
    demo_user_uuid UUID;
BEGIN
    ...
    IF demo_user_uuid IS NULL THEN
        -- Generate a fixed demo UUID for consistency
        demo_user_uuid := '00000000-0000-0000-0000-000000000001'::UUID;
        
        -- Insert demo user if it doesn't exist (this might fail due to auth constraints, which is fine)
        BEGIN
        ...
        EXCEPTION WHEN OTHERS THEN
            -- Ignore errors - we'll use NULL user_id instead
            demo_user_uuid := NULL;
        END;
    END IF;

    -- Insert some initial vocabulary words using the demo user UUID (or NULL)
...
-- Success message
SELECT 'Database update completed successfully! Added vocabulary support with proper UUID handling.' as message;